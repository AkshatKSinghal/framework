...
class AWBBatch extends BaseController
{
    ...
    private function updatePersistentStore()
    {
        ...
        while (!feof($availableFile)) {
            $availableAwb = trim(fgets($availableFile));
            if (array_key_exists($availableAwb, $logAwb)) {
                echo 'in printing';
                $event = $logAwb[$availableAwb];
                switch ($event) {
                    case 'used':
                ...
                file_put_contents($insertFileName, $availableAwb . PHP_EOL, FILE_APPEND);
            }
        }
        foreach ($fileTypes as $fileType) {
